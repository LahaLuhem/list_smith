...
class CompareRow:
    ...
    `scenario` carries its pivot when it has one (`sync_search_scaling[list_size=100000]`), so each
    ...
    delta_pct: float
    p_value: float
    # Pooled within-side variation: the run-to-run noise this delta has to clear.
